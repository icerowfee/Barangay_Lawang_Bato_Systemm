...
<head>
    ...
    <title>Admin Generated Report</title>
</head>
...
<body class="flex h-screen bg-gray-100" x-data="{
    ...
    @if (Auth::guard('admin')->user()->role == '1')
        <div class="relative h-screen flex-1 p-8 overflow-y-scroll" x-data="{
            ...
            <h2 class="text-4xl font-bold mb-8">Generated Reports</h2>
                ...
                <div class="relative flex justify-between gap-4 items-center">
                    <div class="flex gap-4">
                        <div x-init="$nextTick(() => { if (search) $refs.searchInput.focus(); })">
                            ...
                            <input x-ref="searchInput" type="text" placeholder="Search name or email..."
                                class="border p-2 rounded w-64" x-model.debounce.500ms="search"
            ...
            @forelse ($generatedReports as $report)
                <a href="#"
                    @click=" reportPath = '{{ asset('storage/' . $report->path) }}'; viewGeneratedReport = true"
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600 mt-1">
                                Generated by: <span class="font-medium">
                                    ...
                                    {{ $adminUser->where('id', $report->reporter_id)->first()->firstname . ' ' . $adminUser->where('id', $report->reporter_id)->first()->lastname ?? 'N/A' }}</span>
                                |
                                ...
                                <div class="text-sm text-gray-500 mt-2">
                                    ...
                                    <ul class="list-disc list-inside">
                                        @foreach (json_decode($report->filters, true) as $key => $value)
                                            <li><span class="capitalize">{{ str_replace('_', ' ', $key) }}</span>:
                                                {{ $value }}</li>
                                        @endforeach
                                    </ul>
                                </div>
    ...
    <div class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50" x-cloak
        x-show="viewGeneratedReport" x-data="{}" @click.away="viewGeneratedReport = false">
        ...
        <div
            ...
            <template x-if="reportPath">
                <iframe :src="reportPath" class="flex-1 w-full rounded-lg border" frameborder="0">
                </iframe>
            </template>
        </div>
    </div>
