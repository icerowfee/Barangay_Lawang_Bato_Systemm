...
<div x-cloak x-show="openArchive || activeTab === 'job-referral-request-archive-tab'"
    ...
    <div class="relative bg-white rounded-lg shadow-md w-3/4 min-h-[77%] overflow-hidden">
        ...
        @if ($archivedCompanyAccounts->isEmpty())
        ...
        @else
            <div>
                <table class="min-w-full table-fixed text-base bg-white shadow-md overflow-hidden">
                    ...
                    <tbody>
                        @foreach ($archivedCompanyAccounts as $archivedRequest)
                            <tr class="cursor-pointer odd:bg-gray-50 even:bg-white hover:bg-slate-200 transition text-center"
                                @click="selectedRequest = JSON.parse(atob('{{ base64_encode(json_encode($archivedRequest)) }}')); showArchivedCompanyAccountsData = true;">
                                <td class="border-b p-3 truncate">
                                    {{ ($archivedCompanyAccounts->currentPage() - 1) * $archivedCompanyAccounts->perPage() + $loop->iteration }}
                                </td>
                                <td class="border-b p-3">{{ $archivedRequest->company_name }}</td>
                                <td class="border-b p-3">{{ $archivedRequest->business_type }}</td>
                                <td class="border-b p-3">{{ $archivedRequest->barangay }},
                                    {{ $archivedRequest->city }}</td>
                                <td class="border-b p-3 truncate max-w-[130px]">{{ $archivedRequest->formatted_date }}
                                </td>
                                <td class="border-b p-3 truncate max-w-[100px]">{{ $archivedRequest->formatted_time }}
                                </td>
                                <td
                ...
                <!-- Popup Modal for archived company accounts -->
                <div x-cloak x-show="showArchivedCompanyAccountsData"
                    ...
                    <div class="bg-white rounded-lg shadow-lg w-2/5 max-h-[80vh] overflow-y-auto">
                        ...
                        <div class="relative bg-[#0F5192] text-white px-4 py-3 flex-1 items-center">
                            <h2 class="text-2xl font-bold p-2 text-center">REVIEW ARCHIVED COMPANY ACCOUNT</h2>
                            <button
                                class="absolute top-6 right-6 text-[18px] font-semibold text-white hover:text-red-600 w-[27px]"
                                @click="selectedRequest = null; showArchivedCompanyAccountsData = false">
                                ✕
                            </button>
                        </div>
                        ...
                        <div class="flex justify-center align-middle gap-6 py-6">
                            {{-- <form action="{{ route('approve.company.account.request') }}"
                                                method="POST">
                                                @csrf
                                                @method('PUT')
