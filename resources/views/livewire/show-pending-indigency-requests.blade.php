<!-- Popup Table for New Indigency Requests -->
<div wire:poll x-cloak x-show="openIndigency || activePopup === 'new-indigency-request-tab'"
    ...
    <div class="relative bg-white rounded-lg shadow-md w-3/4 min-h-[77%] overflow-hidden">
        <div class="relative bg-[#0F5192] text-white px-4 py-4 flex-1 items-center">
            <h2 class="text-3xl font-bold text-center self-center">New Indigency Requests</h2>
        </div>
        @if ($newIndigencyRequests->isEmpty())
            {{-- AYUSIN MO TO --}}
            <div class="w-full flex items-center justify-center h-[637px]">
                <div class=" text-center mb-10">
                    ...
                    <!-- Subtitle -->
                    <p class="text-gray-500">New indigency applications will appear here as
                        they come in.</p>
                </div>
            </div>
            <div class="absolute bottom-0 flex w-full items-center p-8 pt-6 space-x-6">
                ...
                <!-- Close Button -->
                <button @click="openIndigency = false; activePopup = ''"
        ...
        @else
            <div>
                <table class="min-w-full table-fixed text-base bg-white shadow-md overflow-hidden">
                    ...
                    <tbody>
                        @foreach ($newIndigencyRequests as $newRequest)
                            <tr class="cursor-pointer odd:bg-gray-50 even:bg-white hover:bg-slate-200 transition text-center"
                                @click="selectedRequest = JSON.parse(atob('{{ base64_encode(json_encode($newRequest)) }}')); showNewIndigencyRequests = true;">
                                <td class="border-b p-3 truncate">{{ $loop->iteration }}</td>
                                <td class="border-b p-3 truncate">
                                    {{ $newRequest->representative_lastname }},
                ...
                <!-- Popup Modal for new indigency requests-->
                <div x-cloak x-show="showNewIndigencyRequests"
                    ...
                    <div class="bg-white rounded-lg shadow-lg w-2/5 h-fit overflow-y-hidden">
                        ...
                        <div class="relative bg-[#0F5192] text-white px-4 py-3 flex-1 items-center">
                            <h2 class="text-2xl font-bold p-2 text-center">REVIEW NEW INDIGENCY REQUEST</h2>
                            <button
                                class="absolute top-6 right-6 text-[18px] font-semibold text-white hover:text-red-600 w-[27px]"
                                @click="selectedRequest = null; showNewIndigencyRequests = false"
                        ...
                        <div class="flex justify-center align-middle gap-6 py-6">
                            ...
                            <!-- Accept -->
                            <form wire:submit.prevent="approveIndigencyRequest"
                                ...
                                <input type="hidden" name="activePopup" value="new-indigency-request-tab"> --}}
                                <button
            ...
            <div class="absolute bottom-0 flex w-full items-center p-8 pt-6 space-x-6">
                <div class="flex-1">
                    {{ $newIndigencyRequests->appends(request()->except('pending_indigency_page'))->links() }}
                </div>
